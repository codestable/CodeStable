...
独立 issue 关闭后，可以把稳定结论直接回写 project spec。探索型 issue 关闭时，把用户确认过的当前结论和当前理解提炼进 project spec。epic issue 关闭后，先回写 epic spec。只有当用户确认整个 epic 完成，AI 才把 epic spec 中毕业的结论合并回 project spec。
...
- `type: explore`：先确认用户认可 issue 内当前结论和当前理解，再把毕业结论提炼成人能读懂的 spec 内容；错误讨论、代码证据、证据流水、已排除理解和仍未知问题留在 issue。
