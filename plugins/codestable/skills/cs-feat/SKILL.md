...
出现任何一条，走设计对齐再动手：起草方案要点（改什么、契约变化、取舍、影响面）→ 用 `cs-code-review` 的 design review 做独立审查（修复-复审最多 2 轮，超限连分歧一起上交）→ 交用户确认。信号清单：
...
- 改动完成后默认用 `cs-code-review` 做独立审查，仅文案级微小改动可说明后跳过；审查发现的 blocking 项未解决不得宣称完成。

## 收尾

- 报告：做了什么、改动文件、验证结果、遗留事项。
- 需要跨会话继续、多人交接或用户要求留痕时，写一个 `.codestable/work/{slug}.md`（目标 / 现场 / 边界 / 证据 / 验收 / 状态与未决六节，随进展更新；"状态与未决"记录当前进度与待用户确认项，供跨会话恢复；完成后压缩要点进项目文档或 lessons 并删除）。
