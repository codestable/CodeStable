...
所有步骤完成后用下面模板汇报。普通模式下**停下来等用户 review**；Goal 模式下把汇报写入报告 / 状态 / 证据记录后继续进入 before_review gates 和 `cs-code-review`，不等用户。
...
### 第一性原则 pre-pass 核对
{外部行为 / 不可破约束 / 最小充分改动 / 必须不写的东西；说明是否有想加但最终删掉或记为顺手发现的内容}
...
普通模式汇报后停等 review。Goal 模式汇报后不停等用户，除非命中 goal handoff 条件。
...
coding feature / coding step 适合自动化测试时，按同目录 `tdd.md` 的 step 内 TDD 策略执行。它只约束当前 step 里的测试-实现微循环，不改变 `cs-feat` implementation 阶段的 gate、checklist、证据、review / QA 流程。
...
- [ ] 完成汇报已输出；普通模式用户 review 通过（或 review-fix / qa-fix 汇报已输出，等待重跑 `cs-code-review`）；Goal 模式已把汇报写入报告 / 状态 / 证据记录并按 goal 协议继续
- [ ] 没有未处理的"需要叫停"信号
...
普通模式告诉用户："所有步骤完成，方案 doc 已同步。下一步阶段 2.5 代码审查，进入 cs-code-review。"
...
- review-fix 修完后跳过 `cs-code-review` 直接进入 `cs-feat` acceptance 阶段
- qa-fix 修完后跳过 `cs-code-review` 或 `cs-feat` QA 阶段直接进入 `cs-feat` acceptance 阶段
- 关键场景清单一条都没落证据
