# Feature Design Review Protocol
...
2. checklist 存在，`feature` 与目录一致，`steps` 非空，`checks` 非空；缺 checklist 时报告 `blocked`，回 `cs-feat` design 阶段 生成 review candidate checklist。
...
按 feature design review 的严重度语义输出：blocking / important / nit / suggestion / learning / praise / residual-risk。
