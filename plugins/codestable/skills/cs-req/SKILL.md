---
name: cs-req
description: "能力愿景维护。触发：起草/回填/更新 requirements，或 design/acceptance 需要 req delta。不要用于单个功能实现方案(cs-feat design)、大需求拆解排期(cs-epic)、开放式发散讨论(cs-brainstorm)、领域术语/ADR建模(cs-domain)。"
contracts:
  - grep: "backfill"
  - grep: "变更日志"
  - grep: "status: draft"
