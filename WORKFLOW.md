...
每个入口共用同一执行主线：**理解相关事实 → 行动 → 相称的验证 → 交付结果**。风险每次按当前事实重判，不写入持久 lane；触发升级信号（公开契约 / 数据 / 权限 / 真实取舍 / 大范围 diff / 用户要求）时先对齐设计再动手：方案要点经独立 agent design review（修复-复审最多 2 轮，超限连分歧一起上交）后交用户确认。design 与整体验收的确认不可被模型自主跳过；改动完成默认独立 review；声称完成必须附可核验证据。
