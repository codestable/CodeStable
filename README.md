...
<p>
  ...
  <img src="https://img.shields.io/badge/skills-20-6366F1?style=flat-square" alt="Skills"/>  <img src="https://img.shields.io/badge/license-MIT-10B981?style=flat-square" alt="License"/>
</p>
...
- **`cs-complain`** —— 行为不符合预期时，固定预期/实际/复现/证据并生成 bug issue
- **`cs-design`** —— 针对单个 issue 做实现设计：模块归属、接口/数据/状态、测试面和实施顺序
- **`cs-test`** —— 可选测试设计：公司或用户需要时，为单个 issue 写测试目标、用例和执行方式
- **`cs-do`** —— 按 issue 的实现设计写代码、验证，并回写执行记录
- **`cs-close`** —— 关闭 issue：把毕业内容沉淀到 requirements、notes、facts 或 tools
...
requirements 按渐进式披露组织：入口先写综述和子模块导航；复杂子系统、子步骤或领域规则拆成同目录下的独立 requirements 文件，每个子文件也用“背景 → 要实现什么 → 关键规则 → 边界和取舍”的说明姿态写。

- **`cs-requirements`** —— 当前需求说明、领域词汇、关键规则和取舍（背景 / 功能 / 边界 / 为什么需要灵活性）。不引用代码位置、不记历史叙事

### 二者怎么咬合

事项是增量，requirements 是这些增量沉淀出的当前需求真相。**一个 issue 或 epic 关闭时，把毕业的约束、事实和取舍回写 requirements。** requirements 不记历史（那在关闭的 issue 里），issue 不长期描述当前需求。执行纪律（`cs-do`）和知识笔记（`cs-keep`）服务两者。
...
<tr><td><b>讨论入口</b></td><td><code>cs-talk</code></td><td>想法模糊时的讨论 + 整理：聊清楚后写入 <code>talks/</code></td></tr>
<tr><td><b>投诉入口</b></td><td><code>cs-complain</code></td><td>行为不符合预期时，固定预期 / 实际 / 复现 / 证据并生成 bug issue</td></tr>
<tr><td><b>计划入口</b></td><td><code>cs-plan</code></td><td>读取 <code>talks/</code>，判断直接进入 issue 还是先进入 epic，并生成对应草案</td></tr>
<tr><td><b>设计入口</b></td><td><code>cs-design</code></td><td>面向单个 issue 做实现设计，写回模块归属、接口/数据/状态、测试面和实施顺序</td></tr>
...
<tr><td><b>关闭入口</b></td><td><code>cs-close</code></td><td>关闭 issue，并把稳定结论沉淀到 requirements / notes / facts / tools</td></tr>
...
<tr><td rowspan="3"><b>辅助资料</b></td><td><code>cs-maketools</code></td><td>人带 AI 跑通未知流程，沉淀 notes、facts 引用和可选 tools</td></tr>
<tr><td><code>cs-keep</code></td><td>坑点 / 技巧 / 决策 / 调研沉淀到 <code>notes/</code>，纯 markdown，全文检索</td></tr>
<tr><td><code>cs-note</code></td><td>一两行启动必读事实追加到 <code>facts.md</code></td></tr>
...
═══════════════════════════════════════════════════════════════
   ...
   搭骨架              整理 talks → 生成事项 → 设计 issue → 可选测试 → 执行验证 → 关闭沉淀
                       cs-complain ─▶ 行为跑偏时生成 bug issue
═══════════════════════════════════════════════════════════════
 事项 · 要做、做完会关闭的事           （.cs/issues/ 或 .cs/epics/）
───────────────────────────────────────────────────────────────
   cs-plan   ──▶ 从 talks 判断：直接 issue，或先进 epic 再拆 issues
   cs-complain ─▶ 行为不符合预期时，生成可关闭的 bug issue
   cs-design ──▶ 针对单个 issue 做实现设计（模块 / 接口 / 数据 / 测试）
   cs-test   ──▶ 可选测试设计（目标 / 用例 / 层级 / test-first）
   cs-do     ──▶ 按 issue 写代码、验证、回写执行记录
   cs-close  ──▶ 关闭 issue，把毕业内容沉淀到长期实体
...
───────────────────────────────────────────────────────────────
   cs-requirements ──▶ 综述 + 子模块说明 + 领域词汇 + 取舍
                       （背景 / 功能 / 关键规则 / 边界）
                       北极星：读它即知代码背后的需求与取舍
═══════════════════════════════════════════════════════════════
            ▼ 任意时刻"这个值得记下来" ▼
 辅助资料 · 知识沉淀（复利工程）
   ...
   cs-keep ──▶ .cs/notes/   纯 markdown，全文检索
   cs-note ──▶ .cs/facts.md  一两行启动必读事实
═══════════════════════════════════════════════════════════════
```
...
│   │   ├── REQUIREMENTS.md   # 入口（lazy）
│   │   ├── REQUIREMENTS-MAP.md # 多 requirements 拓扑入口（仅大型项目）
│   │   └── {slug}.md         # 一块一篇
│   │
│   ├── notes/                # 知识笔记，纯 markdown，全文检索（cs-keep）
│   │   └── YYYY-MM-DD-{slug}.md
│   │
...
- `requirements/` 用人话说明当前需求、约束、领域词汇和取舍；入口写综述，细节按子模块渐进披露；历史在关闭的 issue 里
- issue 默认写入本地 `issues/YYYY/MM/{slug}.md`，避免一个目录堆太多文件；查找时递归搜索 `issues/`
- `notes/` 是知识笔记，纯 markdown 无 frontmatter，靠全文检索——好写好搜
...
各 cs 行动技能开始前都先确认 `cs` 技能：当前对话或执行上下文已经读过就复用；没读过就读一次，拿到实体和技能边界。
